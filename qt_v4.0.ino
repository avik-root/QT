...
const unsigned long doubleTapWindow = 400; // ms window for double-tap
...
bool alarmActive = false;         // true when alarm is currently firing
...
String pageHead(String title, int active) {
  String h = "<!DOCTYPE html><html><head><meta charset='UTF-8'>"
    ...
    ".nav a{color:#8b949e;text-decoration:none;padding:12px 10px;font-size:12px;font-weight:600;border-bottom:2px solid transparent;transition:.2s}"
    ...
    "input[type=text],input[type=password],input[type=number],input[type=time],select,textarea{width:100%;padding:10px;background:#0d1117;border:1px solid #30363d;border-radius:8px;color:#c9d1d9;font-size:14px;outline:none}"
    ...
    ".alarm-row{display:flex;gap:8px;align-items:center;padding:12px 0;border-bottom:1px solid #21262d}"
    ".alarm-row:last-child{border:none}"
    ".alarm-time{flex:1}"
    "</style></head><body><div class='nav'>";
  String p[] = {"Home","WiFi","API","Display","Alarm"};
  String u[] = {"/","/wifi","/api","/display","/alarm"};
  for(int i=0;i<5;i++) h+="<a href='"+u[i]+"'"+(i==active?" class='ac'":"")+">"+p[i]+"</a>";
...
void connectSTA() {
  ...
  Serial.println("[STA] Connecting to: " + sta_ssid);
...
void checkWifiReconnect() {
  ...
  // Always verify AP is alive — even in AP-only mode
  if(WiFi.softAPIP() == IPAddress(0,0,0,0)) {
    Serial.println("[AP-DEBUG] AP lost, restarting...");
    WiFi.enableAP(true);
    WiFi.softAP(ap_ssid, ap_password, 1, 0, 4);
    delay(500);
    Serial.println("[AP-DEBUG] AP restored: " + WiFi.softAPIP().toString());
  }

  // WiFi mode check — ensure we're still in AP_STA
  if(WiFi.getMode() != WIFI_AP_STA) {
    Serial.println("[AP-DEBUG] WiFi mode changed! Restoring AP_STA...");
    WiFi.mode(WIFI_AP_STA);
    WiFi.setSleep(false);
    WiFi.softAP(ap_ssid, ap_password, 1, 0, 4);
    delay(500);
  ...
  if(sta_ssid.length()==0) return; // skip STA reconnect if no SSID
...
// ==================== ALARM SYSTEM ====================
void checkAlarm() {
  if(alarmActive) return; // already firing
  if(!timeSynced) return;
  struct tm t;
  if(!getLocalTime(&t)) return;
  for(int i=0;i<MAX_ALARMS;i++){
    if(alarms[i].enabled && alarms[i].hour==t.tm_hour && alarms[i].minute==t.tm_min && t.tm_sec==0){
      alarmActive=true;
      buzzerState=false;
      lastBuzzerToggle=millis();
      Serial.println("[ALARM] Alarm " + String(i) + " triggered! " + String(alarms[i].hour) + ":" + String(alarms[i].minute));
      break;
    }
  }
}
...
void dismissAlarm() {
  ...
  buzzerState=false;
  digitalWrite(BUZZER_PIN, LOW);
  Serial.println("[ALARM] Dismissed by double-tap");
}

void drawAlarmScreen() {
  // Called when alarm is active — shows time + alarm indicator
  ...
  // "ALARM!" header with blinking
  display.setTextSize(1);
  if(buzzerState) {
    display.setCursor(30,0);display.print("!! ALARM !!");
  } else {
    display.setCursor(42,0);display.print("ALARM");
  }
  ...
  display.setCursor((SCREEN_WIDTH-w)/2-8,18);display.print(timeBuf);
  display.setTextSize(1);
  display.setCursor((SCREEN_WIDTH-w)/2+w-4,18);display.print(ampm);
  // Date
  const char* days[]={"SUN","MON","TUE","WED","THU","FRI","SAT"};
  char dateBuf[16]; strftime(dateBuf,sizeof(dateBuf),"%d %b %Y",&t);
  display.setCursor(0,38);display.print(days[t.tm_wday]);
  display.setCursor(30,38);display.print(dateBuf);
  // Dismiss instruction
  display.drawLine(0,50,127,50,SSD1306_WHITE);
  display.setCursor(8,54);display.print("Double-tap to dismiss");
  display.display();
}
...
void drawTimeScreen() {
  ...
  // Touch hint
  display.setCursor(20,57);display.setTextSize(1);display.print("Touch to switch");
  display.display();
}
...
void handleTouch() {
  ...
  // Debounce: ignore if too soon after last registered tap
  if(touched && !lastTouchState && (now - lastTouchTime > touchDebounce)) {
    lastTouchTime = now;
    tapCount++;
    if(tapCount == 1) {
      firstTapTime = now;
  ...
  // Process taps after the double-tap window expires
  if(tapCount > 0 && (now - firstTapTime > doubleTapWindow)) {
    if(tapCount >= 2) {
      // DOUBLE TAP detected
      if(alarmActive) {
        dismissAlarm();
        Serial.println("[TOUCH] Double-tap: alarm dismissed");
      } else {
        // Double-tap when no alarm = advance two screens
        nextScreen();
        lastRotation = now;
        Serial.println("[TOUCH] Double-tap: skip screen -> " + String(curScreen));
      }
    } else {
      // SINGLE TAP detected
      if(!alarmActive) {
        nextScreen();
        lastRotation = now; // reset auto-rotation timer
        Serial.println("[TOUCH] Single-tap: screen -> " + String(curScreen));
      }
      // Single tap during alarm does nothing (need double-tap)
    }
    tapCount = 0;
  }
}
...
void handleHome() {
  ...
  // Alarm status
  h+="<div class='row'><div class='box'><div class='lbl'>Alarm</div><div class='val'>"
...
void handleAlarm() {
  ...
  for(int i=0;i<MAX_ALARMS;i++){
    ...
    h+="<div class='alarm-row'>"
      "<div class='alarm-time'>"
      "<label>Alarm "+String(i+1)+"</label>"
      "<input type='time' name='at"+String(i)+"' value='"+String(timeVal)+"'>"
      "</div>"
      "<label class='sw' style='margin-top:28px'>"
      "<input type='checkbox' name='ae"+String(i)+"'"+String(alarms[i].enabled?" checked":"")+">"
  ...
  for(int i=0;i<MAX_ALARMS;i++){
    if(alarms[i].enabled){
      char buf[6]; sprintf(buf,"%02d:%02d",alarms[i].hour,alarms[i].minute);
      int dispHr=alarms[i].hour%12; if(dispHr==0)dispHr=12;
      const char* ap=alarms[i].hour>=12?"PM":"AM";
      h+="<div style='padding:6px 0;border-bottom:1px solid #21262d'>"
        "<span style='color:#58a6ff;font-weight:600'>Alarm "+String(i+1)+"</span>"
        "<span style='color:#c9d1d9;margin-left:10px'>"+String(dispHr)+":"+String(buf+3)+" "+String(ap)+"</span>"
        "<span class='tag tg' style='margin-left:8px'>ON</span></div>";
    }
  }
  h+="</div></div>";
  // Instructions
...
void handleAlarmSave() {
  for(int i=0;i<MAX_ALARMS;i++){
    String timeStr = server.arg("at"+String(i)); // "HH:MM"
    if(timeStr.length()>=5){
      alarms[i].hour = timeStr.substring(0,2).toInt();
      alarms[i].minute = timeStr.substring(3,5).toInt();
    }
    alarms[i].enabled = server.hasArg("ae"+String(i));
    // Save to preferences
    prefs.putInt(("ah"+String(i)).c_str(), alarms[i].hour);
    prefs.putInt(("am"+String(i)).c_str(), alarms[i].minute);
    prefs.putBool(("ae"+String(i)).c_str(), alarms[i].enabled);
    Serial.println("[ALARM] Saved alarm " + String(i) + ": " + String(alarms[i].hour) + ":" + String(alarms[i].minute) + " " + String(alarms[i].enabled?"ON":"OFF"));
  }
  server.sendHeader("Location","/alarm");server.send(303);
}
...
void setup() {
  ...
  // Load alarm settings
  for(int i=0;i<MAX_ALARMS;i++){
    alarms[i].hour = prefs.getInt(("ah"+String(i)).c_str(), 0);
    alarms[i].minute = prefs.getInt(("am"+String(i)).c_str(), 0);
    alarms[i].enabled = prefs.getBool(("ae"+String(i)).c_str(), false);
    if(alarms[i].enabled){
      Serial.println("[ALARM] Loaded alarm " + String(i) + ": " + String(alarms[i].hour) + ":" + String(alarms[i].minute));
    }
  }
...
void loop() {
  ...
  // If alarm is active, lock display to alarm screen
  if(alarmActive) {
    drawAlarmScreen();
    delay(10);
    return; // skip normal screen rotation
  }
