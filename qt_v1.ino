...
void connectSTA() {
  ...
  if(WiFi.status()==WL_CONNECTED){
    ...
    delay(1000);
    struct tm t;
    if(getLocalTime(&t,5000)) timeSynced=true;
  } else {
    staConnected=false;
...
// ==================== WEATHER ====================
void fetchWeather() {
  if(!staConnected||apiKey.length()==0||city.length()==0) return;
  HTTPClient http;
  ...
  if(weatherValid&&wLat!=0){
    String aUrl="http://api.openweathermap.org/data/2.5/air_pollution?lat="+String(wLat,4)+"&lon="+String(wLon,4)+"&appid="+apiKey;
    http.begin(aUrl);
    code=http.GET();
    if(code==200){
      String body=http.getString();
...
void handleHome() {
  ...
  h+="<div class='card'><h2>Status</h2><div class='row'>"
...
void setup() {
  ...
  display.setCursor(10,10);display.println("QT");
  display.setTextSize(1);display.setCursor(15,35);display.println("Multi Display");
  display.setCursor(25,50);display.println("Starting...");
  display.display();delay(1500);

  prefs.begin("oled-srv",false);
